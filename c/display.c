...
void sdl_display(const char* filename) {
    ...
    // Create a window
    SDL_Window* window = SDL_CreateWindow("Image Viewer", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, imageWidth, imageHeight, SDL_WINDOW_SHOWN);
    if (window == NULL) {
    ...
    // Event loop to keep the window open until user closes it
    SDL_Event event;
    int quit = 0;
